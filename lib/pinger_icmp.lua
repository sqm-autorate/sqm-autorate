            ...
            if (string.byte(data, hdr_len + 1) == 14) then
                ...
                if reflector_list then
                    ...
                    if (pos > 0 and src_pkt_id == pkt_id) then
                        ...
                        util.logger(util.loglevel.TRACE, "Exiting receive_icmp_pkt() with stats return")
...
function M.send(reflector, pkt_id)
    ...
    util.logger(util.loglevel.TRACE, "Entered send_icmp_pkt() with values: " .. reflector .. " | " .. pkt_id)
